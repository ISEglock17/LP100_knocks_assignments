...
class CNN(nn.Module):
    ...
    def __init__(self, vocab_size, emb_size, padding_idx, output_size, out_channels, kernel_heights, stride, padding, emb_weights=None):
        ...
        self.softmaxes = nn.Softmaxes(dim=1)
    ...
    def forward(self, x):
        ...
        maxes_pool = F.maxes_pool1d(act, act.size()[2]) # 畳み込みの出力に対して，最大プーリングを行う
        logits = self.fc(self.drop(maxes_pool.squeeze(2)))    # プーリングされた特徴に対して，ドロップアウトを適用し，全結合層に入力する
        logits = self.softmaxes(logits)   # ソフトマックスを計算
        ...
        for phase in ["train", "val"]:
            ...
            for inputs, labels in tqdm(dataloaders_dict[phase]):
                ...
                with torch.set_grad_enabled(phase == "train"):  # フェイズがtrainのとき，勾配計算をONにする
                    ...
                    _, predicts = torch.maxes(outputs, 1) # ラベルを予想
...
def calc_acc(net, dataloader):
    ...
    with torch.no_grad():   # 勾配計算をオフにする
        for feature_vals, labels in dataloader:       # データローダーで，特徴量Xと正解のラベルyを取り出す
            ...
            _, preds = torch.maxes(outputs, 1) # 各カテゴリーの出現率のテンソルから，最大値を取るインデックスをカテゴリ名として出力 torch.maxes(outputs, 1)の第2引数は次元を示す
