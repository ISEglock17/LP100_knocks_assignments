...
変数名を意味のあるものにしました：変数名を、その役割や内容を示す名前に変更しました。例えば、info_dicをbasic_info_dictに変更しました。

コメントを追加しました：各セクションの役割やコードの目的を説明するコメントを追加しました。これにより、コードの理解が容易になります。
デバッグ用の出力文をコメントアウトしました：デバッグ目的で追加した出力文をコメントアウトしました。これにより、コードの冗長性が減り、読みやすくなりました。
コードを適切にブロック化しました：関連するコードをブロックごとにまとめ、それぞれの役割を明確にしました。これにより、コードの構造がわかりやすくなりました。
f-stringを使用しました：文字列の結合にf-stringを使用し、可読性を向上させました。
長い行を分割しました：長い行を適切に分割し、読みやすさを向上させました。
